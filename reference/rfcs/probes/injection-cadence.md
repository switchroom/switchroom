...
- **Live latency refutes the design's "0.6–0.75s".** The live recall log
  ...
  reference/rfcs/design-v2.md cites "0.6–0.75s" three times (lines 182, 504, 1060) — the fourth of the four sites the frontmatter says were corrected has since had its own 0.6–0.75s mention removed entirely, not just re-scoped —
...
Step 6a (reference/rfcs/design-v2.md:1043–1154) keeps per-turn injection as the fleet default —
...
hybrid-hardened` is the default tier value (reference/rfcs/design-v2.md:1165–1173). Only 6b
removes injection, per-agent, explicitly "gated on evidence we do not have"
(reference/rfcs/design-v2.md:1156–1157). So the operator's steer is substantially already conceded
...
itself names as "pull-miss risk... real and accepted" (reference/rfcs/design-v2.md:580–582)
...
tokens, gains trust, quality sign unknown" (reference/rfcs/design-v2.md:585–586) — that is not
a case, it is an absence of one.

### F3. What actually still holds the anti-injection position up, post-E-57

- **Token cost:** §6 (reference/rfcs/design-v2.md:1278–1369) honestly brackets the net as
...
- **Benchmark:** E-57 downgraded to noise (reference/rfcs/design-v2.md:1335–1338). Dead.
...
fragment injection is removed regardless", reference/rfcs/design-v2.md:373–374; tools-only as the
...
(reference/rfcs/design-v2.md:153–156) already partially concedes it (withdrew "our instance is
...
**Factual error found (STALE — already corrected by the design itself):** this probe originally cited design-v2.md:1146 for the claim "the variant both chat-domain comparators actually ship — observation-only, **1024-token-capped**". That exact line no longer exists — not merely shifted: the design's current text at `reference/rfcs/design-v2.md:160–162` already reads "observation-only, capped (openclaw 1024, Hermes 4096 tokens — P-05, corrected rev 10; rev 9's 'both 1024' misread Hermes)", i.e. this exact error was fixed in the design at rev 10, independently of this probe. Kept for record; no live target to re-anchor to. Hermes's
...
§8.2a (reference/rfcs/design-v2.md:1545–1560) admits the E-79 winning arm "re-injected the
