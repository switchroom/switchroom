...
When the user asks about scheduled tasks, cron jobs, timers, or what runs automatically — show them the full picture: active systemd timers AND the schedule entries from switchroom.yaml.
...
```
!`systemctl --user list-timers "switchroom-*" --no-pager 2>/dev/null || echo "no timers found"`
```

## Step 1 — Parse the timer output

For each timer in the output above, extract:
- Timer name (e.g. `switchroom-assistant-cron-0.timer`)
- **Next** fire time
- **Last** fire time
- **Passed** (how long ago it last ran)
- Service unit name

## Step 2 — Read switchroom.yaml for prompts

Find and read `~/.switchroom/switchroom.yaml` (or `$SWITCHROOM_CONFIG`). For each schedule entry in the file, show:
...
Match each systemd timer to its switchroom.yaml entry. A timer named `switchroom-<agent>-cron-<N>` corresponds to the Nth schedule entry for `<agent>`.
...
- Timer exists in systemd but not in switchroom.yaml (stale, should be removed with `switchroom agent reconcile`)
- Entry in switchroom.yaml but no systemd timer (not installed, run `switchroom agent reconcile <name>`)
...
journalctl --user -u switchroom-<agent>-cron-<N>.service --no-pager -n 20

# Manually trigger a task now
systemctl --user start switchroom-<agent>-cron-<N>.service

# Check if a timer is enabled
systemctl --user is-enabled switchroom-<agent>-cron-<N>.timer
```
