...
anyone else's solution (E-91). Cross-bank reads are explicit fan-out
(E-32). **Correction, this pass (E-97): fleet-wide common config does
NOT ship as a bank template onto existing banks** — E-49 pitches
`/v1/bank-template-schema` for onboarding a *new* bank, and that route
is GET-only on the deployed engine; there is no apply-to-existing-banks
verb, no cross-bank shared-directive primitive at all, and §5 step 3's
"mechanical create+deactivate pass" is withdrawn as REFUTED (§5 step 3,
review m-5). Fleet-common guardrails stay per-bank rows, reconciled one
directive at a time via the step-2 shim where they've drifted
(`windows-boxes-access-and-full-stop`, the one confirmed case).
...
directive count. **Live at 176 active directives fleet-wide (E-98),
three banks already at or past the WARN line: overlord 26, marko 26,
assistant 25, klanker 24** — the watch this paragraph asks for is not
hypothetical.

**Correction, this pass (E-97): fleet-common guardrails are NOT
consolidated to one source of truth via a bank template — that
primitive does not exist on the deployed engine (§5 step 3, REFUTED).**
The working assumption had been that `no-confabulation` — pasted
verbatim into every bank — carried the same drift risk as
`windows-boxes-access-and-full-stop`, and that a template-and-deactivate
pass would fix both at once for ~1KB/injection, zero behaviour change.
A live audit of all 15 banks (E-98) found neither premise holds:

- `no-confabulation` is live in **14** banks (not the 13 previously
  cited — a fifteenth, `switchroom-dev`, carries zero directives of any
  kind), and every one of the 14 copies is **byte-identical** (1,029
  chars, priority 10, active). There is no drift to reconcile and
  nothing to deduplicate even if a cross-bank primitive existed.
- `windows-boxes-access-and-full-stop` is the one genuinely drifted
  case (E-43), and it is only **two** copies: klanker 2,362 chars,
  overlord 3,567 chars, overlord's text a strict superset of klanker's.
  The remedy is unchanged and still cheap — a single-directive `PATCH`
  on klanker's copy via the step-2 shim — but it is a one-directive fix,
  not a template operation, and it saves nothing beyond itself.

The `~1KB/injection saved fleet-wide, zero behaviour change` claim is
withdrawn in full. What remains true: `disposition_*` values are
audited per bank, not templated (E-40: klanker's disposition is already
populated by nobody — audit before leaning on it), and the token saving
this design counted from consolidation is only available through
shrinking or retiring directives (step 4's triage), never through
cross-bank dedup.
...
1. **Probes and instrumentation (no behaviour change).**
   ...
   **Implemented in #4738** (open, `step1-instrumentation` → `main`):
   `injected_directive_ids()` in
   `vendor/hindsight-memory/scripts/lib/directives.py`, the pure
   counterpart of the existing `count_omitted_directives()`, returns the
   post-cap head-slice actually rendered into the prompt, in priority
   order; wired into both `_write_recall_log` call sites in `recall.py`
   as a new `directive_ids` field — real ids on a fresh recall, `null`
   on a cache hit, matching the existing `directive_count`/
   `directives_omitted` null-on-cache-hit convention (no directives
   block is built on a cache hit). Covered by 5 new unit tests
   (`tests/test_directives.py`) and 3 new integration tests
   (`tests/test_recall_integration.py`); `python3 -m unittest discover`
   reports 1079 tests OK.
   (d) Fix the stale scores comment at `recall.py:344-345` (E-24).
   **Already shipped, correction (this pass):** this item is stale —
   the comment was deleted in commit `70d29067` ("Deleted the stale
   'Hindsight's HTTP API does not return similarity scores' comment …
   and rewrote the #475 gate rationale"), which predates this document.
   The `recall.py:344-345` anchor has since drifted with the file and
   now points at unrelated recall-cache-lookup code (`entry.get
   ("saved_at")` / `entry.get("context")`) — a second line-anchor drift
   of the same class E-53 already named as a ledger convention; E-24 is
   corrected to cite the commit instead (below). `grep -rn similarity
   vendor/hindsight-memory/scripts/` returns nothing relevant on `main`
   today. **Also already shipped, found at the same pass and not
   previously listed as a step-1 item here:** per-memory injected score
   aggregates — `_injected_score_stats()`
   (`vendor/hindsight-memory/scripts/recall.py:531`) already computes
   `injected_score_min/median/max` and both `_write_recall_log` call
   sites already carry them (real values on cache miss, `None` on cache
   hit), covered by `tests/test_recall_min_score.py` and
   `tests/test_recall_integration.py`. Verified against `origin/main`
   at `4a70ee58`, the commit this document's own history branches from
   — nothing to build for either.
...
3. **Bank template for fleet-common guardrails — REFUTED (this pass,
   ...
   `no-confabulation` copies and reconcile the drifted
   `windows-boxes-access-and-full-stop` pair (E-43) via the step-2 shim.
   Standalone win claimed: ~1KB per injection saved fleet-wide
   immediately, zero behaviour change. **That primitive does not exist
   on the deployed engine, and the zero-behaviour-change claim is
   false.** The live route table (`GET /openapi.json`,
   `127.0.0.1:18888`, re-verified this pass) has exactly one
   bank-template route, `/v1/bank-template-schema`, and it is **GET
   only** (`POST` → 405) — a schema an agent reads, not a template it
   applies to an existing bank. Directives live at
   `/v1/default/banks/{bank_id}/directives` (GET, POST) and
   `/v1/default/banks/{bank_id}/directives/{directive_id}` (GET, PATCH,
   DELETE) — per-bank rows, full stop; there is no cross-bank
   shared-directive primitive anywhere in the schema. So "deactivate the
   13 per-bank copies" leaves every one of those banks carrying **zero**
   copy of a rule they currently all obey — a behaviour change, and the
   opposite of the claimed zero-risk win. **The ~1KB/injection,
   zero-behaviour-change saving is WITHDRAWN.** What a live audit of all
   15 banks actually found (E-97, E-98): all 14 `no-confabulation`
   copies are byte-identical (1,029 chars, priority 10, active) — so
   there is no drift to reconcile there, and nothing to dedup even if
   the primitive existed. The one thing that IS real and still stands:
   the `windows-boxes-access-and-full-stop` pair is genuinely drifted,
   but it is only **two** copies (klanker 2,362 chars, overlord 3,567
   chars, overlord's text a strict superset of klanker's) — a
   single-directive `PATCH` on klanker's copy via the step-2 shim,
   still cheap, still reversible, not a template operation. The token
   saving this design counted from step 3 is only available through
   shrinking or retiring directives (step 4's triage), never through
   cross-bank deduplication. Reversible: N/A — not built, not
   attempted.
...
- **Principle checks:** defaults improve with no operator action (step 1
  adds measurement at zero behaviour change; **correction, this pass:**
  step 3's claimed ~1KB/injection saving is withdrawn — REFUTED, no
  bank-template-apply primitive exists on the deployed engine, §5 step
  3 — so the "steps 1–3" saving this checklist previously cited no
  longer holds; the single-directive `windows-boxes` fix inside the old
  step 3 still stands but was never the source of that number); rides
  existing machinery (hooks, doctor, approval cards, cron, the preserved
