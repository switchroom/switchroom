...
- `allow_once` — single use. **This is what the primary `Allow` button binds to.** The row is burned on its first successful match: `evaluateDecisionRow` claims it with a conditional `UPDATE ... WHERE id = ? AND revoked_at IS NULL` (an atomic compare-and-swap — of two racing uses exactly one is granted, the loser is denied) and stamps `revoke_reason = 'allow_once_consumed'`. Because `lookupDecision` filters `revoked_at IS NULL`, a burned row leaves the scope-keyed fast path entirely and the next attempt re-prompts rather than being denied outright.

  **Known residual gap (scope-keyed fast path).** A hook that times out waiting for the operator (`src/cli/drive-write-pretool.ts:349`) leaves an approved-but-unspent decision behind: the operator's tap lands after the hook gave up, so the grant is recorded with nobody to consume it. The *next* write to that scope then spends it via the fast path with no card shown. This is a real gap and it is NOT closed by single-use burning — but burning bounds it precisely: **exactly one** stale write can ride a late `allow_once`, where previously every subsequent write to that scope was auto-approved forever. Closing it properly needs the fast path to correlate by `request_id` (as `lookupDecisionByRequestId` already does) rather than by scope.
