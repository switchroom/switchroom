...
description: Shows which switchroom agents are running, their uptime, and current state. Use when the user asks about status, 'what's running', uptime, 'are my agents OK', or wants an overview of all agents.
---

# Agent Status

When the user asks about agent status, what's running, or whether their agents are OK, run this skill to show a live overview of all configured switchroom agents.

## Live Agent Data

Current output from `switchroom agent list`:

```
!`switchroom agent list --json 2>/dev/null || echo "unavailable"`
```
...
2. If the output is "unavailable" or empty, tell the user `switchroom` isn't on PATH and suggest:
   ```
   npm install -g switchroom-ai
   # or check that switchroom is on your PATH
   ```
