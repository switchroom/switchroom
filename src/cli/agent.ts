 ...
 */
/**
 * Files the build writes to the source tree as a deterministic function
 * of HEAD. They show up dirty in `git status` after every `npm run build`,
 * but they don't represent un-reviewed code — running the build again on
 * any clean checkout regenerates them identically. Filter them from the
 * dirty-tree check so a `bun run dev -- agent restart all` doesn't
 * trip the guard purely because the operator just rebuilt.
 *
 * Keep this list MINIMAL — every entry weakens the guard. Only add a
 * file if (a) the build script writes it on every invocation, AND
 * (b) the content is purely a function of HEAD + build environment.
 * If a file fails (b) — i.e., its contents could carry uncommitted
 * intent — it does NOT belong here.
 */
const DIRTY_IGNORE_PATHS: ReadonlySet<string> = new Set([
  "src/build-info.ts", // regenerated by scripts/build.mjs from `git rev-parse HEAD`.
]);

/**
 * Parse `git status --porcelain` output and decide whether the working
 * tree is meaningfully dirty (i.e., dirty in any file outside
 * DIRTY_IGNORE_PATHS). Pure function — no I/O, lifted out of
 * `checkSwitchroomBranch` so the filtering rules are unit-testable
 * without spawning git.
 *
 * Porcelain v1 format per line: `XY filename` where XY is the two-char
 * status code. For renamed entries (`R<sp>orig -> new`) we look at the
 * destination path, which is what the user is "modifying."
 */
export function parsePorcelainDirty(porcelain: string): {
  dirty: boolean;
  ...
  for (const raw of porcelain.split(/\r?\n/)) {
    ...
    // Drop the two status chars + the separator space. Renamed entries
    // are `R<sp>old -> new`; we want `new` (the post-rename path).
    const path = raw.slice(3);
    const finalPath = path.includes(" -> ") ? path.split(" -> ")[1] : path;
    if (DIRTY_IGNORE_PATHS.has(finalPath)) {
      ignored.push(finalPath);
    } else {
      meaningful.push(finalPath);
    }
  }
  return {
...
function checkSwitchroomBranch(): string | null {
  ...
  try {
    ...
    if (branch === "main" || branch === "master") {
      ...
      // gateway behaviour). Filter regenerated build artifacts so a
      // post-build `restart all` doesn't false-trip the guard.
      const porcelain = execFileSync("git", ["-C", cwd, "status", "--porcelain"], {
        encoding: "utf-8",
        stdio: ["ignore", "pipe", "ignore"],
      });
      const { dirty, meaningfulPaths } = parsePorcelainDirty(porcelain);
      if (dirty) {
        const preview = meaningfulPaths.slice(0, 3).join(", ");
        const suffix = meaningfulPaths.length > 3 ? ` (+${meaningfulPaths.length - 3} more)` : "";
        return `switchroom checkout has uncommitted changes on ${branch}: ${preview}${suffix}`;
      }
      return null;
    }
...
/**
 * In v0.7 the agent infra layer migrated from per-agent systemd units to
 * docker-compose services. The compose file is regenerated by
 * `switchroom apply` (PR-A1), and `docker compose up -d --remove-orphans`
 * brings containers in sync. The previous in-restart unit-regen helper
 * (`regenerateSystemdUnitsForAgent`) is gone; reconcile now only touches
 * agent-dir files (settings.json, hooks, start.sh) — not infra units.
 */
...
export function registerAgentCommand(program: Command): void {
  ...
  agent
    ...
    .action(
      withConfigError(async (name: string, opts: { profile?: string }) => {
        ...
        // v0.7: agents run as docker-compose services. The compose file
        // is regenerated by `switchroom apply` which is the canonical
        // "bring containers in sync" verb. Per-agent schedules run in
        // the scheduler container, which reads switchroom.yaml directly
        // — no separate timer install step.
  ...
  agent
    ...
    .description("Remove an agent's directory (run `switchroom apply` after to remove the container)")
      ...
      withConfigError(async (name: string, opts: { yes?: boolean }) => {
        ...
        if (!opts.yes) {
          process.stdout.write(
            chalk.yellow(
              `Destroy agent "${name}"? This removes ${agentDir}. (Run \`switchroom apply\` after to take the container down.) [y/N] `
            )
          );
        ...
        // v0.7: agents run as docker-compose services. Removing the
        // entry from switchroom.yaml + running `switchroom apply` (with
        // `docker compose up -d --remove-orphans`) takes the container
