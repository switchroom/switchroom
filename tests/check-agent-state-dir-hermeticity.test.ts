/**
 * Unit tests for `scripts/check-agent-state-dir-hermeticity.mjs`.
 *
 * These assert the gate's OUTCOME — that it fails on each shape of the defect
 * (a runner whose guard wiring was deleted) and passes on the fixed shape —
 * against synthetic tmp repo roots. A gate nobody has seen fail is not a gate.
 */
...
import { checkRepo } from "../scripts/check-agent-state-dir-hermeticity.mjs";

const REPO_ROOT = join(dirname(fileURLToPath(import.meta.url)), "..");
const GUARD = "tests/vitest-setup/agent-state-dir-guard.mjs";
...
function makeRepo(opts: RepoOpts = {}): string {
  const root = mkdtempSync(join(tmpdir(), "state-dir-hermeticity-lint-"));
  roots.push(root);
  ...
  if (opts.guardFiles !== false) {
    for (const f of ["agent-state-dir-guard.mjs", "agent-state-dir-guard-core.mjs"]) {
...
describe("check-agent-state-dir-hermeticity", () => {
  ...
  it("fails when a bunfig preloads something else", () => {
    const r = checkRepo(makeRepo({ rootPreload: ["./tests/other-preload.mjs"] }));
    expect(r.ok).toBe(false);
