...
> **Retired in PR #893 (Phase 4 cron-fold-in).** The singleton
> `switchroom-cron` / `switchroom-scheduler` container described below
> was removed during the cron-fold-in cutover (PRs #890–#893). Cron now
> runs in-container in every agent as a sibling of the gateway,
> delivering fires through the same `InboundMessage` IPC path Telegram
> uses (synthesized turns tagged `meta.source="cron"`). The
> `ghcr.io/switchroom/switchroom-scheduler` image and `Dockerfile.scheduler`
