/**
 * Bootstrap budget: enforces per-file and total character caps on the
 * workspace context files injected into the system prompt, and emits
 * truncation warnings (with signature-based dedupe) when content had to be
 * dropped.
 *
 * Ported from OpenClaw (src/agents/bootstrap-budget.ts). Kept as close to the
 * upstream implementation as possible so we can pull in upstream fixes with
 * minimal conflict.
 */

import path from "node:path";
import type { EmbeddedContextFile, WorkspaceBootstrapFile } from "./bootstrap-types.js";
...
export function buildBootstrapTruncationSignature(
  ...
  const files = analysis.truncatedFiles
    ...
    .sort((a: { path: string; rawChars: number; injectedChars: number; causes: string[] }, b: { path: string; rawChars: number; injectedChars: number; causes: string[] }) => {
...
export function formatBootstrapTruncationWarningLines(params: {
  ...
  lines.push(
    "If unintentional, raise switchroom workspace bootstrapMaxChars and/or bootstrapTotalMaxChars config.",
  );
  return lines;
}
