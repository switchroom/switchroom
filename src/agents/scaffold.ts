...
\`mcp__hindsight__retain(content="switchroom's vitest config must set assetsInclude to ['**/*.yaml'], or a .yaml imported with { type: 'text' } fails to resolve under vitest", bank_id="switchroom-dev", tags=["repo:switchroom", "build"])\`

Passing \`bank_id\` is what makes this the exception: auto-retain can't reach a shared bank (it always resolves your own), so if you don't make the call by hand the knowledge stays trapped in your bank. Two rules keep the shared bank clean: route only durable, repo-scoped facts there — never conversation, user preferences, or anything about a person (those stay in your own bank via ordinary retain) — and tag every one \`repo:<name>\` so it is findable. Writes are always durable and readable by the configured agents; on the read side, ordinary \`recall\` / \`reflect\` reach the shared repo bank only for agents that list it in \`memory.recall.additional_banks\` (a handful of dev agents) — for everyone else the shared write is write-only from their own recall.
