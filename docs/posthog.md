...
| `rate_limit_429_classified` | [telegram-plugin/gateway/gateway.ts](../telegram-plugin/gateway/gateway.ts) (operator-event 429 classifier; payload built by `build429ClassifiedMetric` in [telegram-plugin/throttle-tier.ts](../telegram-plugin/throttle-tier.ts)) | `agent`, `classification` (`account-scoped` \| `litellm-local` \| `generic-transient`), `action` (`throttle` \| `failover` \| `calm`), `reset_at_ms`, `reset_in_ms`, `limit_type`, `limit`, `current_usage` — one per terminal rate-limit-family operator event, fired before the notice cooldown; correlate `account-scoped` fires with fleet TPM to size LiteLLM `tpm_limit` caps (docs/auth.md § LiteLLM-proxy-local 429s) |
