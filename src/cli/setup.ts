...
import { STEP_ACTIVE, STEP_DONE, stepHeader, stepHeaderTo } from "./setup-step-ui.js";
import { stepMemoryBackend } from "./setup-memory-backend.js";

// Step-header presentation helpers live in setup-step-ui.ts so a step
// implemented in its own module (setup-memory-backend.ts) can render the same
// numbered header without importing setup.ts (which would form an import cycle
// and drag setup.ts's vault graph into a module that must stay vault-free).
...
// ─── Step 6: Memory Backend ─────────────────────────────────────────────────
//
// stepMemoryBackend + its MemoryBackendDeps/MemoryBackendOutcome/isMemoryBackendDisabled
// live in setup-memory-backend.ts (imported above). They were extracted so the
// step's vitest test (setup-recall-pool-provision.test.ts) can load without
// setup.ts's transitive vault → bun:sqlite graph, which vitest cannot resolve.
