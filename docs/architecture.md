...
2. **Fleet-management commands** — `/agents`, `/restart <other>`, `/agentstop`, `/agentstart`, `/update`, `/logs`, `/reconcile`, `/grant`, `/dangerous`, `/permissions`, `/vault`, `/memory`, `/topics`. Intercepted by the gateway and routed to the `switchroom` CLI **only when the agent has `admin: true`** in `switchroom.yaml`. Other agents reject with an "admin required" reply; Claude never sees the command.
