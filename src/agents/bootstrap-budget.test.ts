/**
 * Ported from OpenClaw (src/agents/bootstrap-budget.test.ts).
 *
 * The last upstream test (cache-stability vs legacy warning injection) is
 * omitted here until switchroom has a `buildAgentSystemPrompt` equivalent. The
 * behaviour it validates (warning is appended to user turn prompt, not the
 * stable system prompt) is still covered by `appendBootstrapPromptWarning`'s
 * direct tests.
 */
...
import type { WorkspaceBootstrapFile } from "./bootstrap-types.js";
