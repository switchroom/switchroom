...
- **Determinism-first.** Route every rule to the strongest binding layer its
  ...
  Hindsight directive (reflect-only, last resort).
...
- Classify each candidate change by blast radius, graduated autonomy:
  ...
  - **T1 Auto, silent** — only a small, reversible, eval-passing edit to a
    ...
    audit/rollback.
  - **T2 Propose, one-tap** — medium / shared-skill changes, surfaced as a
    pending suggestion (Telegram one-tap; backing store = Linear issue or a
    small per-agent queue file).
...
- [ ] suggestion accept/ignore rate; pending-queue depth
- [ ] alert: idle-turn cost crosses threshold; bad-apply / rollback fires

## Open Questions

- [ ] Gate-first triage: pure deterministic signals or a small model call?
- [ ] Starting defaults: review cadence, repetition threshold (2 vs 3),
      diff-size cap, max pending/auto-applies per day — tuned by measuring on
      `clerk` first.
- [ ] T2/T3 pending surface: Linear issue vs small queue file.
