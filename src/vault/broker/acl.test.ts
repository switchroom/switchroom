 ...
 *
 * Identity is the per-agent socket path (Phase 2a) — the ACL is per-AGENT
 * via `checkAclByAgent`. #1192 removed the legacy per-cron-index `checkAcl`
 * evaluator (which keyed on a `switchroom-<agent>-cron-<i>.service` cgroup):
 * the in-container scheduler produces no such cgroup, so that path was dead.
 *
 * Covers `checkAclByAgent`:
 *   - key in the agent's schedule[*].secrets union → allowed
 *   - key NOT in the union → denied
 *   - cross-agent: agentA cannot read agentB's secrets → denied
 *   - unknown / empty agent name → denied
 *   - config-derived exceptions (bot_token, google slots, MCP secrets,
 *     standing grant) — see the dedicated describe blocks below.
 *
 * Note: there is no "interactive fallback" path. The broker is for
 * agent-driven access only. Interactive `switchroom vault get` reads the
 * vault file directly with the user's passphrase via --no-broker.
 */

import { describe, expect, it } from "vitest";
import { checkAclByAgent, parseGoogleAccountSlotKey } from "./acl.js";
