...
| `rate_limit_429_classified` | [telegram-plugin/gateway/gateway.ts](../telegram-plugin/gateway/gateway.ts) (operator-event 429 classifier; payload built by `build429ClassifiedMetric` in [telegram-plugin/throttle-tier.ts](../telegram-plugin/throttle-tier.ts)) | `agent`, `classification` (`account-scoped` \| `litellm-local` \| `generic-transient`), `action` (`throttle` \| `failover` \| `calm` — what the gateway DECIDED, emitted pre-execution; the actual failover fire is dedup-gated, so several `failover` decisions inside one dedup window can map to a single real roll), `reset_at_ms`, `reset_in_ms`, `limit_type`, `limit`, `current_usage` — one per terminal rate-limit-family operator event, fired before the notice cooldown; correlate `account-scoped` fires with fleet TPM to size LiteLLM `tpm_limit` caps (docs/auth.md § LiteLLM-proxy-local 429s) |
| `litellm_local_429_notice` | [telegram-plugin/gateway/litellm-local-notice-wiring.ts](../telegram-plugin/gateway/litellm-local-notice-wiring.ts) (payload built by `buildLitellmLocalNoticeMetric` in [telegram-plugin/litellm-local-notice.ts](../telegram-plugin/litellm-local-notice.ts)) | `agent`, `suppressed_count`, `window_ms` — one per litellm-local throttle notice actually ISSUED to ≥1 chat (the debounced "fleet token limiter engaged" message; an empty allowFrom or an all-failed broadcast emits nothing and leaves the debounce window unarmed, so the next event retries); `suppressed_count` is how many litellm-local 429s the cooldown window silently absorbed since the previous notice (0 on the first), `window_ms` the resolved window (default 15 min, `channels.telegram.litellm_notice.window_ms`). Ratio against the `litellm-local` `rate_limit_429_classified` count shows the debounce's effectiveness |
