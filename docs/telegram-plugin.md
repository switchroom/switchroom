...
- Fires only on `mcp__hindsight__create_directive` (📌 remembered) and `mcp__hindsight__invalidate_memory` / the `switchroom memory demote` tag path (✂️ forgot). In practice only the `invalidate_memory` branch can fire today: hindsight 0.8.5 has no tag-write path for an existing memory (`update_memory` accepts no `tags`/`add_tags`), so the demote branch is dead-but-forward-compatible — it lights up unchanged if upstream adds one. See `docs/configuration.md` → "Demoting individual memories from auto-recall".
