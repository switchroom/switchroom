...
**Per-agent `.claude/`** — each agent has a fully isolated Claude config directory at `~/.switchroom/agents/<agent>/.claude/` on the host, bind-mounted into the container at `/agent/.claude/`. Separate auth credentials, separate `settings.json`, separate plugin config, separate transcript store. Everything under the agent home must be owned by the agent's container UID (`allocateAgentUid`): `switchroom apply` enforces this with a chown sweep (`alignAgentUid`), and reconcile — which runs as root on the rollout path — restores it with its own end-of-run sweep and fails loudly if any file it touched is left unreadable by the agent (#3168).
...
2. **Fleet-management commands** — `/agents`, `/restart <other>`, `/agentstop`, `/agentstart`, `/update`, `/logs`, `/reconcile`, `/grant`, `/dangerous`, `/permissions`, `/vault`, `/memory`, `/topics`, `/audit`. Intercepted by the gateway and routed to the `switchroom` CLI **only when the agent has `admin: true`** in `switchroom.yaml`. Other agents reject with an "admin required" reply; Claude never sees the command.
